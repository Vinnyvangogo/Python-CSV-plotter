...
class CdaqPlotViewer(tk.Tk):
    def __init__(self, initial_file=None):
        ...
        self.lines = {}                  # col -> Line2D (on the single shared axes)
    ...
    def _build_layout(self):
        ...
        self.status_var = tk.StringVar(
            value="No file loaded.  |  Click the plot to freeze a cursor; click again to move it."
        )
        ...
        # --- Right panel: toolbar + single shared plot ---
    ...
    def _init_cursor_artists(self):
        ...
        self.cur_text = self.ax.text(
            ...
            bbox=dict(boxstyle="round", fc=BG, ec=BORDER, alpha=0.9), zorder=7
        )
        self.cursor_locked = False
        self.cursor_idx = None
    ...
    def _toggle_channel(self, col):
        ...
        if is_on:
            if col not in self.lines:
                ...
                line, = self.ax.plot(self.df[self.x_col], self.df[col], label=col, linewidth=1, color=color)
                self.lines[col] = line
                marker, = self.ax.plot([], [], "o", color=LOCK_COLOR, markersize=6,
        ...
        # keep an already-frozen cursor's readout in sync with the newly (de)selected channel
    ...
    def _on_motion(self, event):
        ...
        if self.df is None or event.inaxes != self.ax:
